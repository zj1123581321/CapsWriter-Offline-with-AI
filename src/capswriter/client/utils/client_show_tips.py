...
from .client_cosmic import console
...
5. 按 `Ctrl + Shift + C` 退出程序


特性：

1. 完全离线、无限时长、低延迟、高准确率、中英混输、自动阿拉伯数字、自动调整中英间隔
2. 热词功能：可以在 `data/hot-en.txt data/hot-zh.txt data/hot-rule.txt` 中添加三种热词，客户端动态载入
3. 日记功能：默认每次录音识别后，识别结果记录在 `年份/月份/日期.md` ，录音文件保存在 `年份/月份/assets` 
4. 关键词日记：识别结果若以关键词开头，会被记录在 `年份/月份/关键词-日期.md`，关键词在 `data/keywords.txt` 中定义
...
3. 默认的快捷键是 {Config.shortcut}，你可以打开 `config.py` 进行修改
4. MacOS 无法监测到 `caps lock` 按键，可改为 `right shift` 按键
5. 要退出程序，请按 `Ctrl + Shift + C`，不要直接关闭终端
    ''')
    ...
    console.print(f'\n退出程序快捷键：[red bold]Ctrl + Shift + C[/red bold]')
