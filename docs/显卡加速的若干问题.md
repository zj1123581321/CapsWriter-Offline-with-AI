...
FunASRNanoGGUFArgs(
    ...
    llm_use_gpu=True,           # GGUF 部分是否用 GPU，默认 False（或用 CW_LLM_USE_GPU=1）
...
主线（master）默认**全 CPU**，方便 Windows 开箱即用。GPU 部署（如 Linux + CUDA/Vulkan）**无需改动 `config_server.py`**，启动前设置环境变量即可覆盖：

| 环境变量 | 默认 | 取值 | 作用引擎 |
|---|---|---|---|
| `CW_ONNX_PROVIDER` | `CPU` | `CPU` / `CUDA` / `DML` / `TRT` | SenseVoice、FunASR、Qwen |
| `CW_LLM_USE_GPU` | `0` | `0` / `1` | FunASR、Qwen（GGUF 解码器） |
| `CW_VULKAN_FORCE_FP32` | `0` | `0` / `1` | FunASR（Intel 集显精度溢出时设 `1`） |
| `CW_ALIGNER_ONNX_PROVIDER` | `CPU` | 同上 | ForceAligner（独立开关） |
| `CW_ALIGNER_LLM_USE_GPU` | `0` | `0` / `1` | ForceAligner（独立开关） |

> - Paraformer 不支持 GPU，固定 CPU，不受 `CW_ONNX_PROVIDER` 影响。
> - 对齐器调用稀疏、GPU 收益有限，故拆成独立变量，不被主开关裹挟。

Linux GPU 部署示例：

```bash
export CW_ONNX_PROVIDER=CUDA
export CW_LLM_USE_GPU=1
python start_server.py
```

环境变量未设置时回退到 `config_server.py` 的默认值（CPU）；引擎侧另有降级保护：所配 Provider 在 `onnxruntime` 不可用时会静默回退 CPU。
...
对于使用 llama.cpp 解码器的引擎（**Fun-ASR-Nano**、**Qwen-ASR**、**ForceAligner**），GGUF 模型默认 **CPU**（主线开箱即用），可在源码改 `True` 或用环境变量 `CW_LLM_USE_GPU=1` 开启：

```python
llm_use_gpu = True       # 将 GGUF 层全部卸载到 GPU（默认 False）
```
