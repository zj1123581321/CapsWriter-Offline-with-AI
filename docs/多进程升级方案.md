...
在 `message_handler` 函数中添加（或在模块级别初始化）:

```python
# 在 ws_recv 函数开始处初始化分配器
async def ws_recv(websocket):
    # ... 现有代码 ...

    # 初始化任务分配器
    from .server_cosmic import Cosmic
    dispatcher = TaskDispatcher(Cosmic.queues_in)
...
```python
# 使用分配器分配任务
worker_id = dispatcher.dispatch(task)
...
**说明**：
- 将原来的 `queue_in.put(task)` 改为 `dispatcher.dispatch(task)`
- 分配器内部会选择正确的队列
- 可选添加调试日志
...
async def main():
    ...
    # 获取进程数配置
    num_workers = max(1, Config.worker_processes)
    ...
    # 创建多个输入队列（每个进程一个）
    manager = Manager()
    Cosmic.queues_in = [manager.Queue() for _ in range(num_workers)]
...
| `server/utils/server_cosmic.py` | 队列改为列表 | +2 行 |
| `server/utils/server_ws_recv.py` | 使用分配器 | +15 行 |
| `server/utils/server_init_recognizer.py` | 无需修改 | 0 行 |
| `server/core.py` | 多进程创建逻辑 | +60 行 |

**总改动量**: 约 **137 行代码**（新增 + 修改）
